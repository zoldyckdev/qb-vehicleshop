        ...
        if isPointInside then
            ...
            CreateThread(function()
                while insideShop do
                    ...
                    -- Fetch vehicle stock from server
                    QBCore.Functions.TriggerCallback('qb-vehicleshop:server:GetVehicleStock', function(stockCount)
                        vehicleMenu = {
                            {
                                isMenuHeader = true,
                                icon = 'fa-solid fa-circle-info',
                                header = getVehBrand():upper() .. ' ' .. getVehName():upper() .. ' - $' .. getVehPrice(),
                                txt = "Stock: " .. stockCount 
                            },
                            {
...
local function isBlacklistedCategory(shop, category)
    local shopBlacklist = Config.BlacklistedCategories[shop]
    if not shopBlacklist then return false end

    for _, cat in ipairs(shopBlacklist) do
...
RegisterNetEvent('qb-vehicleshop:client:vehCategories', function(data)
    ...
    for k, v in pairs(QBCore.Shared.Vehicles) do
        if type(v.shop) == 'table' then
            for _, shop in pairs(v.shop) do
                if shop == insideShop and (not Config.FilterByMake or v.brand == data.make) then
                    if not isBlacklistedCategory(insideShop, v.category) then
                        catmenu[v.category] = v.category
                        if not firstvalue then firstvalue = v.category end
                    end
                end
            end
        elseif v.shop == insideShop and (not Config.FilterByMake or v.brand == data.make) then
            if not isBlacklistedCategory(insideShop, v.category) then
                catmenu[v.category] = v.category
                if not firstvalue then firstvalue = v.category end
            end
    ...
    if Config.HideCategorySelectForOne and tablelength(catmenu) == 1 then
        TriggerEvent('qb-vehicleshop:client:openVehCats', {
            catName = firstvalue,
            make = Config.FilterByMake and data.make,
            onecat = true
        })
        return
    end
